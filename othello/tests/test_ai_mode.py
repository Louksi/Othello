...
class TestAIMode:
    ...
    def ai_mode(self):
        ...
        return AIMode(board_size=BoardSize.EIGHT_BY_EIGHT, ai_color=Color.BLACK, depth=3)

    def test_init_with_color_string(self):
        """Test initialization with color as string"""
        # Test with lowercase string
        ai_mode = AIMode(board_size=BoardSize.EIGHT_BY_EIGHT,
                         ai_color="black", depth=3)
        assert ai_mode.ai_color == Color.BLACK

        # Test with uppercase string
        ai_mode = AIMode(board_size=BoardSize.EIGHT_BY_EIGHT,
                         ai_color="WHITE", depth=3)
    ...
    def test_get_ai_move(self, mock_find_best_move, ai_mode):
        ...
        mock_find_best_move.assert_called_once_with(
            ai_mode.board, ai_mode.depth, ai_mode.ai_color, True, ai_mode.algorithm, ai_mode.heuristic
        )
        assert result == (3, 4)
    ...
    def test_play_ai_turn(self, mock_input, mock_print, mock_switch_player,
                          mock_process_move, mock_get_ai_move, mock_display_possible_moves,
                          mock_check_game_over, mock_get_possible_moves, mock_display_board,
                          ai_mode):
        """Test play method for AI's turn"""
        # Set up the mocks
        mock_check_game_over.return_value = False
        mock_get_possible_moves.return_value = Bitboard(
            0x0000000810000000)  # Some dummy bitboard
        ...
        with patch('sys.exit') as mock_exit, patch('othello.game_modes.CommandParser') as mock_parser:
            # Set up to exit after one iteration
            mock_input.side_effect = ["quit"]
            mock_parser.return_value.parse_str.return_value = (
    ...
    @patch('builtins.print')
    @patch('builtins.input')
    def test_play_player_turn(self, mock_input, mock_print, mock_switch_player,
                              mock_process_move, mock_display_possible_moves,
                              mock_check_game_over, mock_get_possible_moves, mock_display_board,
                              ai_mode):
    ...
    @patch('builtins.print')
    @patch('builtins.input')
    def test_play_invalid_move(self, mock_input, mock_print, mock_display_possible_moves,
                               mock_check_game_over, mock_get_possible_moves, mock_display_board,
                               ai_mode):
        ...
        with patch('othello.game_modes.CommandParser') as mock_parser_class:
            ...
            with patch.object(ai_mode, 'process_move', return_value=False) as mock_process_move:
                # Mock sys.exit to prevent actual exit
                with patch('sys.exit') as mock_exit:
                    # Set up to exit after processing one move
                    ...
                    assert not hasattr(
                        mock_exit, 'called') or not mock_exit.called
...
def test_play_forfeit(self, mock_input, capsys):
    ...
    with patch('othello.game_modes.AIMode.__init__', side_effect=lambda self, *args, **kwargs: self.__patch_init__(*args, **kwargs)):
        ai_mode = AIMode(ai_color="black")
        # Set current player to human
        ai_mode.ai_color = Color.BLACK
        ai_mode.current_player = Color.WHITE
